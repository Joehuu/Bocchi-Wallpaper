...
const Lyrics = (props) => {
  ...
  React.useEffect(() => {
    const updateCurrentTime = () => setCurrentTime(props.audioRef.current.currentTime * 1000);
    props.audioRef.current.addEventListener("timeupdate", updateCurrentTime);
    return () => props.audioRef.current.removeEventListener("timeupdate", updateCurrentTime);
  }, [])

  React.useEffect(() => {
    // ensure current time is zero
    setCurrentTime(0);
    fetch(`./assets/lyrics/${toFilename(SongData[props.songIndex].name)}.lrc`)
      .then(res => res.text())
